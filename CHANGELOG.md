...
- **README**: the skill table's file-edit extension lists now match each skill's frontmatter (`web-patterns`, `web-security`, `web-performance`, `apply-design-system`). The rows still abbreviate the keyword triggers; the frontmatter is authoritative.
